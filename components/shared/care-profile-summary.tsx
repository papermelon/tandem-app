...
import { Apple, HeartPulse, Info, Phone, ShieldCheck } from "lucide-react";
...
export function CareProfileSummary({ profile, phone, compact = false, className }: Props) {
  const hasProfile = profile && Array.isArray(profile.sections) && profile.sections.length > 0;
  if (!hasProfile && !phone) return null;
