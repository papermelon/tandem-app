...
type PatientDraft = Pick<
  CareRecipient,
  "name" | "age" | "relationship" | "country" | "phone" | "context" | "address" | "careProfile"
>;
        ...
        <form className="mt-4 space-y-4" onSubmit={save}>
          <section className="space-y-3">
            ...
            <label className="block">
              ...
              <Input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+65 8123 4567" />
            </label>
            <label className="block">
          ...
          <div className="flex flex-col gap-2">
            ...
            {canRemove ? (
              <Button type="button" variant="ghost" className="w-full text-destructive" onClick={remove}>
                <Trash2 className="size-4" />
                Remove profile
              </Button>
            ) : liveMode ? (
              <p className="px-2 text-center text-xs leading-5 text-muted-foreground">
                Live profiles can be edited here. Removing a live care recipient is intentionally disabled.
              </p>
            ) : null}
          </div>
        </form>
